...
router.patch('/update/:id', async (req, res, next) => {
    try {
        ...
        const skillsArray = skills?.split(",").map(skill => skill.trim()) || defaultJob.skills; // skills is undefined if not provided
...
router.get("/filter/:skills", async (req, res, next) => {
    try {
        ...
        const jobs = await Job.find({ skills: { $in: skillsArray } }).select("name logo position");   // OR method in searching
        res.status(200).json(jobs);
